...
static const hoja_config_s _hoja_config = {
    // On-chip I2C (instance_N matches driver .i2c_instance fields below).
    .i2c = {
        .instance_0 = {
            .enabled      = true,
            .sda_gpio     = 24,
    ...
    // IMU config (type shaped by HOJA_IMU_DRIVER = LSM6DSR). Dual sensors on
    // I2C instance 0, address-selected, averaged together.
    .imu = {
        .channel_count = 2,
        .channel_a = {
            .bus = LSM6DSR_BUS_I2C, .i2c_instance = 0, .select = 1,
            .gyro  = { .invert_y = true },
            .accel = { .invert_y = true },
        },
        .channel_b = {
            .bus = LSM6DSR_BUS_I2C, .i2c_instance = 0, .select = 0,
            .gyro  = { .invert_y = true },
            .accel = { .invert_y = true },
        },
    },
    ...
    .battery_shutdown_percent = 1, // buck-boost lets us run the cell to empty
    ...
    .usb_pid = 0x10DD, // GC Ultimate PID

    // SInput descriptor params.
    .sinput = {
        .gamepad_type       = SINPUT_SDL_GAMEPAD_TYPE_GAMECUBE,
        .face_buttons_style = SINPUT_SDL_GAMEPAD_FACE_STYLE_AXBY,
        .gamepad_subtype    = 0, // GC Ultimate (board-specific)
    },

    // Physical face-button layout (GameCube style).
    .sewn_layout = SEWN_LAYOUT_AXBY,

    // Tournament-lockout macro disabled (set tourney_macro_code to enable).
    .tourney_macro_code = INPUT_CODE_UNUSED,

    // Power button hold → ship mode (single button; set [1] for a combo).
    .shipping_macro_code = { GCU_INPUT_CODE_PWR, INPUT_CODE_UNUSED },

    // Bluetooth pairing: hold Plus at boot or for 3s at runtime.
    ...
    .wlan_force_code     = INPUT_CODE_RB,

    // Shared N64/GameCube joybus line (data on GPIO19). PIO block + SM are
    // allocated dynamically by the driver.
    .joybus = {
        .data_pin = 19,
    },

    // NESBUS line (data/clock/latch on GPIO 19/21/20). PIO block + SMs are
    // allocated dynamically by the driver.
    .nesbus = {
        .data_pin  = 19,
        .clock_pin = 21,
    ...
    // Dual-channel LRA haptics on GPIO 8/9. PWM slices + DMA channels are
    // allocated dynamically by the driver.
    .haptics = {
        .channel_a_pin    = 8,
        .channel_b_pin    = 9,
    ...
    // RGB layout: named groups (name + LED membership), notification group,
    // no dedicated player group, and the input->group key_mappings table.
    .rgb = {
        .groups = {
        ...
        .player_group_index = -1, // no dedicated player group on this board

        .key_mapping_count = 11,
        .key_mappings = {
            { INPUT_CODE_LX_RIGHT, 0 }, // L Stick
            { INPUT_CODE_RX_RIGHT, 1 }, // R Stick
            { GCU_INPUT_CODE_A,    2 }, // A
            { GCU_INPUT_CODE_B,    3 }, // B
            { GCU_INPUT_CODE_X,     4 }, // X
            { GCU_INPUT_CODE_Y,    5 }, // Y
            { INPUT_CODE_UP,       6 }, // D Up
            { INPUT_CODE_DOWN,     7 }, // D Down
            { INPUT_CODE_LEFT,     8 }, // D Left
            { INPUT_CODE_RIGHT,    9 }, // D Right
            { GCU_INPUT_CODE_PWR,  10 }, // Power
        },
    },
};
